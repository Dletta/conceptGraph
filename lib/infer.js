...
function worldBasis (limitParam) {
  ...
  this.tell = function(graph) {
    ...
    for(var state of this.states) {
      ...
      if(!state.blocked){
        ...
        return this.player(graph, state);
      }
    }
  };
  /*
  ...
  this.player = function(graph, state) {
    ...
    if(result.length>0){
      ...
      for(let i=0;i<result.length;i++){
        ...
        return this.opfor(result[i].state.axioms[0], result[i]);
      }
    // if we found nothing
    } else {
      ...
      if(result.length>0){
        ...
        // we found nothing to prove this fact
          ...
          return this.opfor(state.state.axioms[0], state); //send original state to opfor
      }
    }
  };
  /*
  ...
  */
  this.opfor = function(graph, state){
    ...
    var stateLabel = state.state.label;
    //console.log(stateLabel, state.step);
    if(state.step > this.limit) {return this.final(state);}
    state.step +=1;
    ...
    if(result.length>0){
      ...
      for(var i=0;i<result.length;i++){
        ...
        if(result[i].blocked) {
          ...
          return this.opfor(result[i].state.axioms[0], result[i]);// no negation found, restart
        }
      }
    } else {
      ...
      if (result.length>0) {
        ...
        for(var i=0;i<result.length;i++){
          ...
          return this.player(result[i].state.axioms[0], result[i]);// continue until end condition
        }
      } else {
        ...
        if(state.playerPass && state.opponentPass) {
          ...
          return this.player(state.state.axioms[0], state);// continue until end condition
        }
      }
    }
  };
...
function Tree (target, tarStart, source, sourStart) {
        ...
        if(!this.tree.has(tarCon[j])) {
          ...
          for (let k = 0; k < sourCon.length; k++) {
            ...
            if (temp1.label == temp2.label) {
              ...
              if(tarCon.length<sourCon.length){
  ...
  this.finish = function() {
    ...
    var projections = [];
    var temp = new Map();
    // get first level, there should only be one item, but we will iterate in case of more
    var levelZero = this.level[0];
    // iterate over 'roots' and create an array for each of the roots
    for (let i = 0; i < levelZero.length; i++) {
      var maxLength = 1; // need at minimum one array
      // quickly iterate over all the levels and find the one with the longest length
      // create that amount of arrays on the root
      for (let j = 0; j < this.level.length; j++) {
        if(this.level[j].length > maxLength){
          maxLength = this.level[j].length; // if we got more, then increase number
        }
      }
      // set an array for the root with index i as key
      for (let k = 0; k < maxLength; k++) {
        var index = '';
        index += i;
        index += k;
        temp.set(index,[levelZero[i]]);
      }
    }

    for (let i = 1; i < this.level.length; i++) {
      for (let j = 0; j < this.level[i].length; j++) {
        for(let k = 0; k < levelZero.length; k++){
          // k is i from above and j is k from above
          //if what we are looking at's parent is found, add it to their array
          // to ensure we get only one item per parent to the array, we need to remove an item once we
          // find it
          if(this.level[i][j].parent.from == levelZero[k].from && this.level[i][j].parent.to == levelZero[k].to){
            var index = '';
            index += k;
            index += j;
            var arr = temp.get(index)
            arr.push(this.level[i][j]) // push it to array
            this.level[i].splice(j, 1); // remove it as an option
            j--;
          }
        }
      }
    ...
    for (let i = 0; i < levelZero.length; i++) {
      ...
      for (let k = 0; k < maxLength; k++) {
        ...
        index += i;
        index += k;
        projections.push(temp.get(index));
      }
    }
    return projections;
  };
...
var no2Owners = parse('No two Owners::[Owner:Lard*x](agent?x?y)[Buy:15*y](agent?z?y)[Owner:Leed*z]'); // <-- Change back to stars
neg.add(no2Owners);
