...
function InfOp (graph, axioms, mode) {
  ...
  this.projection = function (graph1, graph2, rule) {
    ...
    for(var i = 0; i<arr1.length;i++){
      for(var j = 0; j<arr2.length; j++) { // <--- This logic makes or breaks the inference
        if(arr1[i].label === arr2[j].label &&
          (arr2[j].referent == '*' || arr2[j].referent == undefined)) {
          score += 1;
          result.concepts.push(arr2[j]);
        } else if (arr1[i].referent == arr2[j].referent) { //the same essentially
          score -= 1;
        }
      }
    }
