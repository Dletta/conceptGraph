...
function InfOp (graph, axioms, mode, stateLabel) {
  ...
  */
  this.join = function(target, source, join, rule) {
    ...
    // a reference keeper for source uuid
    var ref = {};
    var refTarget = {};
    ...
    for(var i=0;i<join.concepts.length;i++){
      ...
      // if there is new information attached from source
        ...
        // make sure the relation we are looking at actually contains
        // the source we joined earlier
        var found = false;
        for (let item in sourceCon.arcs) {
          if(ref.uuid == sourceCon.arcs[item]){
            found = true;
            // change the arc to the target uuid, so that after extend
            // this relation is actually pointing to the target
            sourceCon.arcs[item] = refTarget.uuid;
          }
        }
...
function Tree (target, tarStart, source, sourStart) {
  ...
  this.finish = function() {
    ...
    for (let i = 1; i < this.level.length; i++) { // <---- got issues here
      for (let j = 0; j < this.level[i].length; j++) {
        for(let k = 0; k < levelZero.length; k++){
          ...
          // find it
          if(this.level[i][j].parent.from == this.level[i-1][j].from && this.level[i][j].parent.to == this.level[i-1][j].to){
            var index = '';
            index += k;
            index += j;
