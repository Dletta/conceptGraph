...
function projection (target, source, rule) {
  ...
  for(let i=0;i<targetCon.length;i++){
    for(let j=0;j<sourceCon.length;j++){
      ...
      if( targetCon[i].label == sourceCon[j].label ) {
        posProj.push(new Tree(target, targetCon[i], source, sourceCon[j]));
        found = true;
      }
    }
  }
  ...
  for (var i = 0; i < posProj.length; i++) {
    var temp = posProj[i];
...
function Tree (target, tarStart, source, sourStart) {
  ...
  this.finish = function() {
    ...
    for (let i = 0; i < levelZero.length; i++) {
      ...
      temp.set(i,[levelZero[i]])
    }
/*
    for (let i = 1; i < this.level.length; i++) {
      for (let j = 0; j < this.level[j].length; j++) {
        for(let k = 0; k < levelZero.length; k++){
          //if what we are looking at's parent is found, add it to their array
          if(this.level[i][j].from == levelZero[k].from && this.level[i][j].to == levelZero[k].to){
            var arr = temp.get(k);
            arr.push(this.level[i][j]);
          }
        }
      }
    }
