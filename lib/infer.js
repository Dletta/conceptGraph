  ...
  this.unify = function() {
    ...
    for(var first in this.current.axioms) {
      for(var second in this.current.axioms){
        if(first != second) {
          var proj = this.util.projection(this.current.axioms[first], this.current.axioms[second])
          if(proj.length>0){
            var newGraph = this.util.join(this.current.axioms[first], this.current.axioms[second], proj[0], false);
            //---------------------------------------------  If we joined, we need to remove the second graph and
            // replace the first graph with the new Graph
          }
        }
      }
    }
...
var infUtil = (function(){
  ...
  return {
    ...
    // tar signifies Concept and Graph we join into
    // sou signiefies Concept and Graph we join from
    // side-effect on the graph that is passed in
