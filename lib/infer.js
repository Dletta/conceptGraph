...
function Reasoner (eventSystem) {
  ...
  */
  this.tell = function(graph) {
    ...
    for(var index in this.current.axioms) {
      var proj = this.util.projection(graph, this.current.axioms[index], false);
      if(proj.length > 0) {
        result.push(proj);
      }
    }
  ...
  */
  this.unify = function() {
    var join = false;
    for(var i=0; i<this.current.axioms.length;i++){
      var graph = this.current.axioms[i];
      var truth = new InfOp(graph, this.current, 'schema', 'unify');
      var result = [];
      result = truth.start();
      if(result.length>0){
        console.log(result);
