    ...
    for(let i=0;i<targetCon.length;i++){
      for(let j=0;j<sourceCon.length;j++){
        ...
        if( targetCon[i].label == sourceCon[j].label ) { // <---------------------------------  Here we will check whether types can be changed
          ...
          var tree = new Tree(target, targetCon[i], source, sourceCon[j], rule);
          posProj.push(tree);
          found = true;
        }
      }
...
function Reasoner (eventSystem) {
  // background knowledge
  this.trueNec = new axiomStore('necessarilyTrue');
  this.truePos = new axiomStore('possiblyTrue');
  // laws / rules
  this.law = new axiomStore('law');
  // laws that negate a state
  this.false = new axiomStore('false');
  ...
  */
  this.init = function (necT, posT, law, F, schema) {
    this.trueNec = necT;
    this.truePos = posT;
    this.law = law;
    this.false = F;
  ...
  * Function to tell new information into an existing knowledge base
  * @param graph - Thing with type axiom containing a fact
  * @return string - 4 possible responses 1. True (derivable from Law or necT,posT) 2. False (derivable from F)
  *                   3. Unknown (not derivable), 4. Inconsistent Unknown (derivable from both true and false sets)
  */
  this.tell = function(graph) {
    // make a copy of the graph to preserve the original
    var graphCopy = JSON.parse(JSON.stringify(graph))
    ...
    graph.possibly = false;
    graph.necessarily = false;
    graph.false = false;

    // search through current knowledge to find projections
    // make sure empty var is initialized
    var result = [];
    for(var index in this.truePos.axioms) {
      var proj = this.util.projection(graph, this.truePos.axioms[index], false);
      if(proj.length > 0) {
        result.push(proj);
      }
    }
    ...
    // make sure empty var is initialized
    var result = [];
    for(var index in this.trueNec.axioms){
      var proj = this.util.projection(graph, this.trueNec.axioms[index], false);
      if(proj.length>0){
        result.push(proj);
      }
    }
    ...
    for(var index in this.law.axioms){
      var proj = this.util.projection(graph, this.law.axioms[index], true);
      if(proj.length>0){
        result.push(proj);
      }
    }
    ...
    // search through false to find projection (needs to fully cover false fact)
    // make sure empty var is initialized
    var result = [];
    for(var index in this.false.axioms){
      var proj = this.util.projection(graph, this.false.axioms[index], true);
      if(proj.length>0){
        result.push(proj);
      }
    }
    ...
    if((graph.possibly || graph.necessarily) && graph.false) {
      console.log('Inconsistent');
      return 'Inconsistent Unknown';
    } else if (graph.necessarily && !graph.false) {
      console.log('necessarily true');
      this.trueNec.add(graph);
      this.eventS.send('unify');
      return 'True'
    } else if (graph.possibly && !(graph.necessarily || graph.false)) {
      console.log('possibly true');
      this.truePos.add(graph);
      this.eventS.send('unify');
      return 'True';
    }else if (graph.false && !(graph.possibly || graph.necessarily)) {
      console.log('this statement is false');
      this.false.add(graph);
      return 'False';
    } else {
  ...
  /*
  * Function that unifies anything possible in necessarilyTrue and possiblyTrue set
  * @return boolean - returns true if any joins were executed
  */
  this.unify = function() {
    var join = false;
    for(var i=0; i<this.trueNec.axioms.length;i++){
      var graph = this.trueNec.axioms[i];
      var truth = new InfOp(graph, this.trueNec, 'schema', 'unify');
      var result = [];
      result = truth.start();
      if(result.length>0){
        console.log(result);
    ...
    for(var y=0; y<this.truePos.axioms.length;y++){
      var graph = this.truePos.axioms[y];
      var truth = new InfOp(graph, this.truePos, 'schema', 'unify');
      var result = [];
      result = truth.start();
      if(result.length>0){
        console.log(result);
        // if only one possible state is discovered, we join
        if(result.length == 1){
          this.truePos.axioms[y] = result[0].state.axioms[0];
        }
        join = true;
      }
    }
