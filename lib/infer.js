...
function worldBasis (limitParam) {
  ...
  */
  this.player = function(graph, state) {
    ...
    if(this.step > this.limit) {this.final(state);return;}
      ...
      if(result.concepts.length>1) {
        for(let i=0;i<result.concepts.length; i++){
          ...
          for(let j=0;j<result.concepts.length; j++){
            ...
            if(result.concepts[i].from == results.concepts[j].from){
              ...
              if(result.concepts[i].to != result.concepts[j].to) {
                ...
                result.concepts[j].split = true; // <---------------------------- still need to work out how to do this
        ...
        // create an array to hold split items
        var split =
...
var puzzle = new worldBasis(1000);
