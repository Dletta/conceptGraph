...
function Reasoner () {
  ...
  */
  this.init = function (necT, posT, law, F) {
    this.trueNec = necT;
    this.truePos = posT;
    this.law = law;
    this.false = F;
  ...
  */
  this.tell = function(graph) {
    ...
    var checkPosT = new InfOp(graph, this.truePos, 'schema', 'possibly');
    // make sure empty var is initialized
    var result = [];
    result = checkPosT.start();
    if(result.length>0){
      graph.possibly = true;
    }
    // search through current knowledge to find projections
    var checkNecT = new InfOp(graph, this.trueNec, 'schema', 'necessarily');
    // make sure empty var is initialized
    var result = [];
    result = checkNecT.start();
    if(result.length>0){
      graph.necessarily = true;
    }
    // search through laws to find projection (needs to fully cover law)
    var checkLaw = new InfOp(graph, this.law, 'law', 'laws');
    // make sure empty var is initialized
    var result = [];
    result = checkLaw.start();
    if(result.length>0){
      graph.necessarily = true;
    }
    ...
    if((graph.possibly || graph.necessarily) && graph.false) {
      ...
      console.log(graph.possibly);
      console.log(graph.necessarily);
