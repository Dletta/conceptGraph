...
function Tree (target, tarStart, source, sourStart) {
  ...
  this.growR = function(rule){
    ...
    for (let i = 0; i < expand.length; i++) {
      ...
      console.log('if new information is available then');
        ...
        if(!this.tree.has(tarCon[j])) {
          ...
          for (let k = 0; k < sourCon.length; k++) {
            ...
            if (temp1.label == temp2.label) {
              var more = (sourCon.length>tarCon.length);
              results.push({from:temp2.uuid,to:temp1.uuid,what:'relation',depth:this.depth, parent:expand[i], more:more})
            }
          }
        } else {
          console.log('No new child, if other relation on source, we good');
        }
      }
    }
      ...
      for (var targetI in tarCon) {
        ...
        if(!this.tree.has(tarCon[targetI])) {
          ...
          for (var sourceI in sourCon) {
            // set temp for source relation to compare to
            var temp2 = this.root2G.find('uuid',sourCon[sourceI]); // <------- Check if label is the same first also check if we haven't seen temp2 before proceeding
            // if they are the same, merge them
