...
function extendGraph(target, source, rule){
  ...
  if(rule && source.conConcept.length > 0) {
    console.log('extending Concepts');
    var i = 0;
    var l = source.conConcept.length;
    for(i;i<l;i++){
      console.log(!target.find('uuid', source.conConcept[i].uuid));
      if(!target.find('uuid', source.conConcept[i].uuid)){
        target.addConcept(source.conConcept[i]);
      }
    }
  }
...
function projection(graph1, graph2, rule) {
  ...
  if(rule){
    if(result.concepts.length != v.concept.length){
      console.log(result, 'does not satisfy Hypothesis');
      return;
    }
    //This modifies the rule... don't want to do this.
...
function join(graph1, graph2, join, rule) {
  ...
  //console.log(`joining`);
  if(rule) {
    for(let i=0;i<graph1.conRelation.length;i++){
      graph1.addRelation(graph1.conRelation)
    }
  }
  //console.log(graph1);
  //console.log(graph2);
...
function GOper (graph, axioms, mode) {
  ...
  this.start = function() {
    ...
    if(this.listj.length <= 0){console.log('no possible joins');return;}
